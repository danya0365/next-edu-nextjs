...
import type { LoginViewModel } from '@/src/presentation/presenters/auth/LoginPresenter';
...
export function LoginView({ initialViewModel }: LoginViewProps) {
  const {
    ...
    handleSubmit,
    clearError,
  } = useLoginPresenter(initialViewModel);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            {viewModel?.message || 'เข้าสู่ระบบเพื่อเริ่มเรียนออนไลน์'}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  disabled={loading}
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">จดจำฉันไว้</span>
              </label>
              <Link
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 rounded-lg font-semibold hover:shadow-lg hover:scale-[1.02] transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
